...
export default function AddProductPage() {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const categories = await axiosClient.get(API_ENDPOINTS.GET_CATEGORIES, {
        params: {
  ...
  const fetchProduct = async () => {
    ...
    try {
      const res = await axiosClient.get(
        API_ENDPOINTS.GET_PRODUCT(params.id as string),
      );
  ...
  useEffect(() => {
    ...
    fetchCategories();
  }, [params?.id, webSettings?.tenantId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axiosClient.put(
        API_ENDPOINTS.UPDATE_PRODUCT(params.id as string),
        payload,
      );
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-500">
            Update and publish changes to your product.
          </p>
        </div>
      </div>
